        ...
        <table>
            <thead>
                <tr>
                    ...
                    <th style="width:140px;">Catálogo</th>
                    <th class="text-right" style="width:100px;">Precio</th>
                    <th class="text-center" style="width:80px;">Stock</th>
                </tr>
            </thead>
            <tbody>
                @php $totalProductos = 0; $valorInventario = 0; @endphp
                @foreach($registros as $p)
                    @php $totalProductos++; $valorInventario += (float)($p->precio ?? 0) * (int)($p->stock ?? 0); @endphp
                    <tr>
                        <td class="text-center">{{ $p->id }}</td>
                        <td>{{ $p->nombre }}</td>
                        <td>{{ optional($p->categoria)->nombre ?? '—' }}</td>
                        <td>{{ optional($p->catalogo)->nombre ?? '—' }}</td>
                        <td class="text-right">${{ number_format((float)($p->precio ?? 0), 2) }}</td>
                        <td class="text-center">{{ (int)($p->stock ?? 0) }}</td>
                    </tr>
                @endforeach
            </tbody>
            <tfoot>
                ...
                <tr>
                    <td colspan="4" class="text-right">Valor estimado de inventario</td>
                    <td class="text-right">${{ number_format($valorInventario, 2) }}</td>
                    <td></td>
                </tr>
            </tfoot>
        </table>
